...
export const CallingWidgetScreen = (): JSX.Element => {
  ...
  const [currentLocator, setCurrentLocator] = useState<"queue" | "attendant">(
    "queue"
  );
  const [callLocator, setCallLocator] = useState<CallAdapterLocator>();
  const [userCredentialFetchError, setUserCredentialFetchError] =
  ...
  if (userCredentialFetchError) {
    return (
      <Stack verticalAlign="center" style={{ height: "100%", width: "100%" }}>
        <Spinner
          label={"There was an issue getting credentials"}
          ariaLive="assertive"
          labelPosition="top"
        />
        ;
  ...
  return (
    ...
    >
      <Stack style={{ margin: "auto" }} tokens={{ childrenGap: "1rem" }}>
        ...
        <ul>
          <li>
            Make calls to Teams voice applications like Call Queues and Auto
            Attendants.
          </li>
          <li>
            Adhoc call Teams users with a tenant set that allows for calls from
            your Azure Communication Services resource.
          </li>
        </ul>
        <Text>
          Make the selection to which Teams voice application you would like to
          call with the buttons below. Then use the widget in the corner to
          start your call!
        </Text>
